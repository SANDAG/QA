...
outputfile <- paste("Units_HH_Pop_Vac_Forecast","_Est19","_QA",".xlsx",sep='')
...
households <- calculate_pass_fail(households,.05)
...
allvars <- Reduce(function(x, y) merge(x, y, all=TRUE), 
                  list(households_failed,hhp_failed,hu_failed, hhs_failed, gq_failed,vac_failed))
...
writeData(wb, tableofcontents,x = "Geographies that failed for any variable: Housing units,Households,Household Population, Household size, Group Quarter Population,Vacancy Rate ", startRow = 5, startCol = 2)
...
writeData(wb, summary, x = "List of geographies that failed for any of the following variables: Housing units,Households,Household Population, Household size, Group Quarter Population,Vacancy Rate", 
...
writeData(wb, summary, x = "hu", startCol = 1, startRow = nrow(allvars)+7)
writeData(wb, summary, x = "Number of housing units", startCol = 2, startRow = nrow(allvars)+7)
writeData(wb, summary, x = acceptance_criteria[['hu']], startCol = 3, startRow = nrow(allvars)+7)
...
writeData(wb, summary, x = acceptance_criteria[['hhp']], startCol = 3, startRow = nrow(allvars)+9)

writeData(wb, summary, x = "hhs", startCol = 1, startRow = nrow(allvars)+10)
writeData(wb, summary, x = "Household Size", startCol = 2, startRow = nrow(allvars)+10)
writeData(wb, summary, x = acceptance_criteria[['hhs']], startCol = 3, startRow = nrow(allvars)+10)

writeData(wb, summary, x = "gq", startCol = 1, startRow = nrow(allvars)+11)
writeData(wb, summary, x = "Group Quarter Population", startCol = 2, startRow = nrow(allvars)+11)
writeData(wb, summary, x = acceptance_criteria[['gq']], startCol = 3, startRow = nrow(allvars)+11)

writeData(wb, summary, x = "vac", startCol = 1, startRow = nrow(allvars)+12)
writeData(wb, summary, x = "Vacancy Rate", startCol = 2, startRow = nrow(allvars)+12)
writeData(wb, summary, x = acceptance_criteria[['Vac']], startCol = 3, startRow = nrow(allvars)+12)

addStyle(wb, summary, tableStyle1, rows = (nrow(allvars)+7):(nrow(allvars)+12), cols = 1, gridExpand = TRUE,stack = TRUE)
addStyle(wb, summary, tableStyle2, rows = (nrow(allvars)+7):(nrow(allvars)+12), cols = 2:3, gridExpand = TRUE,stack = TRUE)
...
add_worksheets_to_excel(wb,"hu","blue",8,fullname,acceptance_criteria)
add_worksheets_to_excel(wb,"hh","green",12,fullname,acceptance_criteria)
add_worksheets_to_excel(wb,"hhp","orange",16,fullname,acceptance_criteria)
add_worksheets_to_excel(wb,"hhs","orange",20,fullname,acceptance_criteria)
add_worksheets_to_excel(wb,"gq","yellow",24,fullname,acceptance_criteria)
add_worksheets_to_excel(wb,"vac","purple",28,fullname,acceptance_criteria)
...
for (curr_sheet in names(wb)[3:length(names(wb))]) {
  ...
  addStyle(wb, curr_sheet, style=invisibleStyle, cols=c(ncol(hh_jur)), rows=1:(nrow(hh_cpa)+1), gridExpand=TRUE,stack = TRUE)
