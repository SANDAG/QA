...
#LH 2019 notes
##delete 2010 data because it won't match for total pop and maybe other variables- we need to check to Census
##I think we need to add calculation for vacancy rate in estimates file and add comparison to DOF
##housing unit comparison may need to be added if not in the output file already
...
write.csv(reg_jur_pop_diff[,c("yr_id","Geography","popflag","hhpflag","gqpopflag","hhsflag","hhflag","pop_dof","pop_est","tot_pop_diff","gqpop_dof","gqpop_est","gqpop_diff",
          ...
          "M:\\Technical Services\\QA Documents\\Projects\\Estimates\\4_Data Files\\DOF differences jur & reg differences.csv", row.names=FALSE)
