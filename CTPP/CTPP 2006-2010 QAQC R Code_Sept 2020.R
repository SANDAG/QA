...
# connect to database
channel <- odbcDriverConnect('driver={SQL Server}; server=DDAMWSQL16; database=dpoe_stage; trusted_connection=true')
database_data<- data.table::as.data.table( RODBC::sqlQuery(channel,
paste0("SELECT * 
FROM [dpoe_stage].[fact].[ctpp_2010]
WHERE tbl_id LIKE '%B%'"),  ## CTPP_2010 for 2006-2010 data
...
## importing tables starting with A
file_names <- dir(path = ".", pattern = "CA_2006thru2010_A") #where you have your files
t2data <- do.call(rbind,lapply(file_names,fread))
gc() #release memory

## importing tables starting with B
file_names2 <- dir(path = ".", pattern = "CA_2006thru2010_B") #where you have your files
t2datab <- do.call(rbind,lapply(file_names2,fread))
gc()

# repeat this for t2data and t2databb
source_data <- t2datab
gc() #release memory
...
sum(is.na(dbase_est$est))
gc()

source_est<- source_data[, 3]
...
sum(is.na(source_est$EST)) ## THIS will RESULT in 571057355 NAs 
...
sum(is.na(dbase_5$SE))
source_5<- source_data[,4]
source_5[source_5 == ""] <- NA
source_5$SE<- as.numeric(source_5$SE)
sum(is.na(source_5$SE))

identical(dbase_5, source_5)

source_data<- source_data[order(source_data$EST)]
