...
SELECT o.parcel_id, development_type_id_2018,du_2018,year_simulation,capacity_type, run_id
FROM  [urbansim].[urbansim].[urbansim_lite_output] o
...
ON o.parcel_id = p.parcel_id and capacity_type = 'adu' and run_id = 444	
...
,ROUND(CAST(1.0 * parcels_w_adus/count(p.parcel_id) as float),4) as proportion_of_available
FROM [urbansim].[urbansim].[parcel] p		
...
SELECT uspa.[parcel_id],uspa.[shape],uspa.[parcel_acres],uspa.[mgra_id],uspa.[jurisdiction_id],uspa.site_id
INTO #selection4
FROM #uspa AS uspa
JOIN [ws].[dbo].[CWA_DEMAND_FORECAST_2019] AS cwa ON cwa.[Shape].STIntersects(uspa.[centroid]) = 1
ORDER BY parcel_acres;
SELECT *
FROM urbansim.[urbansim].parcel p
WHERE p.parcel_id IN (select parcel_id from  [urbansim].[urbansim].[urbansim_lite_output] where  run_id = 444 and capacity_type = 'adu') AND
