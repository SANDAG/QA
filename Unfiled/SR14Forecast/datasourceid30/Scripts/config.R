
#LH added datasource_id_current

datasource_id_current <- 30


datasource_ids <- c(17,30)
datasource_names <- c("Series 14 (ds 17)","Series 14 (ds 30)")
datasource_name_short <- c("Series 14","Series 14")
datasource_outfolder <- "vacancy_ds17_and_ds30"

# datasource_ids <- c(28,30)
# datasource_names <- c("Series 14 (ds 28)","Series 14 (ds 30)")
# datasource_name_short <- c("Series 14","Series 14")
# datasource_outfolder <- "vacancy_ds28_and_ds30"

#datasource_ids <- c(13,30)
#datasource_names <- c("Series 13 (ds 13)","Series 14 (ds 30)")
#datasource_name_short <- c("Series 13","Series 14")
#datasource_outfolder <- "vacancy_ds13_and_ds30"
