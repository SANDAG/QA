...
hh_sql = getSQL("../Queries/hh_hhp_hhs_ds_id.sql")
hh_sql <- gsub("ds_id", datasource_id, hh_sql)
hh15<-sqlQuery(channel,hh_sql)
odbcClose(channel)

hh14$ds_id<-14
hh15$ds_id<-15

hh14<- hh14[order(hh14$geotype,hh14$geozone,hh14$yr_id,hh14$ds_id),]
hh14$N_chg <- ave(hh14$hhp, factor(hh14$geozone), FUN=function(x) c(NA,diff(x)))
hh14$N_pct <- (hh14$N_chg / lag(hh14$hhp))*100
hh14$N_pct<-sprintf("%.2f",hh14$N_pct)

hh15<- hh15[order(hh15$geotype,hh15$geozone,hh15$yr_id,hh15$ds_id),]
hh15$N_chg <- ave(hh15$hhp, factor(hh15$geozone), FUN=function(x) c(NA,diff(x)))
hh15$N_pct <- (hh15$N_chg / lag(hh15$hhp))*100
hh15$N_pct<-sprintf("%.2f",hh15$N_pct)

hh_region = subset(hh15,geotype=='region')
hh14 = subset(hh14,geotype=='jurisdiction')
hh15 = subset(hh15,geotype=='jurisdiction')
...
#hh14$ds_id<-NULL
hh14$households<-NULL
hh14$hhs<-NULL

#hh15$ds_id<-NULL
hh15$households<-NULL
hh15$hhs<-NULL
...
hh_jur<-rbind(hh14, hh15)

#setnames(hh14, old =c("hhp","N_chg","N_pct"), new =c("hhp14","N_chg14","N_pct14"))
#setnames(hh15, old =c("hhp","N_chg","N_pct"), new =c("hhp15","N_chg15","N_pct15"))


#hh$geozone<-as.character(hh$geozone)

#hh<-merge(select(hh14,yr_id,geotype,geozone,hhp14),(select(hh15,yr_id,geotype,geozone,hhp15)),by.x=c("yr_id","geotype","geozone"),by.y=c("yr_id","geotype","geozone"),all=TRUE)

#hh<-merge(hh14,hh15,by.x=c("yr_id","geotype","geozone"),by.y=c("yr_id","geotype","geozone"),all=TRUE)
