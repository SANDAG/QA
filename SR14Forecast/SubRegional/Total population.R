...
for(i in jur_list) { #1:length(unique(hh_jur[["cityname"]]))){
  ...
  plot<-ggplot(plotdat,aes(x=yr, y=N_chg,fill=cityname)) +
    ...
    labs(title=paste("Absolute Change: No. of Total Pop\n ", i,' and Region, 2016-2050',sep=''), 
  ...
  # ggsave(plot, file= paste(results, 'Total Pop',  i, ".png", sep=''))#, scale=2)
  ...
  ggsave(output, file= paste(results, 'total pop', i, ".png", sep=''))#, scale=2)
}
