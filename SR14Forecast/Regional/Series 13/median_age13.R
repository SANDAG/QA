...
median_age_cpa_sql = getSQL("../Queries/Queries - 13/median_age_cpa.sql")
...
median_age_cpa<- median_age13_cpa[order(median_age13_cpa$geotype,median_age13_cpa$geozone,median_age_cpa$yr_id),]
median_age_jur<- median_age13_jur[order(median_age13_jur$geotype,median_age13_jur$geozone,median_age_jur$yr_id),]
median_age_region<- median_age13_region[order(median_age13_region$geotype,median_age13_region$geozone,median_age_region$yr_id),]
...
results<-"plots\\Median_Age13\\jur"
...
median_age13_jur$jurisdiction_id<-citynames[match(median_age_jur$geozone, citynames$jur_list2),1]
median_age13_jur$reg<-median_age_region[match(median_age_jur$yr_id, median_age_region$yr_id),3]
...
for(i in 1:length(jur_list)){
  plotdat = subset(median_age13_jur, median_age13_jur$jurisdiction_id==jur_list[i])
   plot<-ggplot(plotdat,aes(x=yr_id, y=median_age13, colour=geozone)) +
     geom_line(size=1)+ 
    geom_line(aes(x=yr_id, y = reg, colour = "Region")) +
    scale_y_continuous(label=comma,limits=c(30.0,47.0))+ 
    labs(title=paste("Median Age13 ", jur_list2[i],' and\n Region, 2012-2050',sep=''), 
         y=" median age", x="Year",
  ...
  ggsave(plot, file= paste(results, 'median_age13', jur_list[i], ".png", sep=''))#, scale=2)
  output_table<-data.frame(plotdat$yr_id,plotdat$median_age13,plotdat$reg)
  setnames(output_table, old=c("plotdat.yr_id","plotdat.median_age13","plotdat.reg"),new=c("Year","Jurisdiction Median Age","Region Median Age"))
  ...
  ggsave(output, file= paste(results, 'median_age13', jur_list[i], ".png", sep=''))#, scale=2)
}
